...
class Spider(RedisSpider):
    ...
    name = "disease_list_spider"
    redis_key = build_redis_keys()["disease_list_url"]

    def parse(self, response):
        html = response.text
        list_urls = []
        if is_first_list_page("disease", response.url):
            list_urls = [url for url in extract_list_urls("disease", response.url, html) if url != response.url]
        ...
        self.logger.info(
            "疾病列表页: %s | 列表入队: %s/%s | 详情入队: %s/%s | 列表队列剩余: %s | 样本详情: %s",
            response.url,
            new_list_count,
            len(list_urls),
            new_doctor_count,
