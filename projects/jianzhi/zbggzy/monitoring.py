...
def build_biz_type(module_name: str, notice_type: str, sub_type: str = "") -> str:
    ...
    - 如果站点存在真实二级分类（如“房屋建筑”“水利”），优先使用：一级类型-二级类型
...
class WebsiteRedisStorage:
    ...
    def store_website_data(
        self, province: str, city: str, website_data: dict[str, Any]
    ) -> bool:
        try:
            ...
            if result > 0:
                logging.info(
                    f"成功存储: website_data_v3/{province}/{city}/{unique_key}"
                )
            else:
                logging.info(
                    f"数据已存在，已更新: website_data_v3/{province}/{city}/{unique_key}"
                )
            return True
        except Exception as exc:  # pragma: no cover
            logging.error(f"存储失败: {province}/{city} - {exc}")
            return False
