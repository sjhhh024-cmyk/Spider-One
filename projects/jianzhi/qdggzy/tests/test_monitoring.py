...
def test_build_biz_type_prefers_real_second_level_type_when_available() -> None:
    assert (
        monitoring.build_biz_type("建设工程", "合同签订公示", "房屋建筑")
        == "建设工程-房屋建筑"
    )
...
def test_build_biz_type_falls_back_to_first_level_type_when_no_real_second_level_type() -> (
    ...
    assert monitoring.build_biz_type("无形资产", "合同签订公示") == "无形资产"


def test_build_logo_info_matches_monitoring_document_fields() -> None:
    logo_info = monitoring.build_logo_info(
        "青岛市公共资源交易电子服务系统", "建设工程-房屋建筑"
    )
...
def test_spider_monitor_flush_uses_document_storage_shape() -> None:
    ...
    assert call["city"] == "青岛市"
    assert call["webname"] == "青岛市公共资源交易电子服务系统"
    assert call["updates"] == {
        "WasSuccessful": 1,
        "WebsiteError": 1,
        "HasContent": 1,
...
def test_monitoring_module_is_local_to_qdggzy_project() -> None:
